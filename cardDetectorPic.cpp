...
// dimenzije cifre v kotu
int RANK_WIDTH = 75;
int RANK_HEIGHT = 105;

// dimenzije grba v kotu
int SUIT_WIDTH = 70;
int SUIT_HEIGHT = 95;

// meje dovoljene razlike ujemanja za cifro in grb
int RANK_DIFF_MAX = 2000;
int SUIT_DIFF_MAX = 1000;

// meje dovoljene ploščine za zaznavanje kart
int CARD_MAX_AREA = 280000;
int CARD_MIN_AREA = 20000;
...
Mat preprocess_image(Mat image) {
    // vrne crno belo, adaptivno thresholdano sliko
...
/*
    ...
    */
    ...
    imshow("Card rank", rank);
    imshow("Card suit", suit);
...
qcard match_card(qcard card, vector<train_image> train_ranks, vector<train_image> train_suits) {
    ...
    if ((card.rank_img.size() != Size(0,0)) && (card.suit_img.size() != Size(0,0))) {
        ...
        for (int i = 0; i < train_ranks.size(); i++) {
            ...
            cout << "Diff: " << train_ranks[i].name << " -> "  << rank_diff << "\n";
        ...
        for (int i = 0; i < train_suits.size(); i++) {
            ...
            cout << "Diff: " << train_suits[i].name << " -> " << suit_diff << "\n";
    ...
    cout << "Best diff: " << best_rank_match_diff << " | " << best_suit_match_diff << "\n";
    cout << "Best Result: " << best_rank_match_name << " | " << best_suit_match_name << "\n";
   ...
   */
    ...
    //while (true) {
        ...
        frame = imread(argv[1], CV_LOAD_IMAGE_COLOR);
            //break;
        resize(frame, show, Size(720, 1080));
